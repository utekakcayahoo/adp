...
Calls the five UC functions behind the MCP tools and asserts the planted answer key
...
equipment fault), the **FAC-006 load creep** (only site above baseline / worst gap), and the
**Phase-7 silent-zero substrate** (future window → all zeros; unknown id → `{}`).
...
- **No model-in-the-loop auto-runner.** The chosen scope is *regression + rubric*. A thin
  `claude_agent_sdk` harness that fires S1–S9 and auto-scores with full MLflow agent traces
