...
-- Auto-exposed at: /api/2.0/mcp/functions/umut_aws_classic_stable_catalog/adp
...
CREATE OR REPLACE FUNCTION umut_aws_classic_stable_catalog.adp.list_facilities()
RETURNS STRING
...
RETURN (
  ...
  FROM umut_aws_classic_stable_catalog.adp.adp_facilities
)
-- @@STATEMENT@@
CREATE OR REPLACE FUNCTION umut_aws_classic_stable_catalog.adp.query_energy(
...
RETURN (
  ...
  FROM (
    ...
    FROM umut_aws_classic_stable_catalog.adp.adp_energy_readings
...
-- @@STATEMENT@@
CREATE OR REPLACE FUNCTION umut_aws_classic_stable_catalog.adp.compute_emissions(
...
RETURN (
  ...
  FROM umut_aws_classic_stable_catalog.adp.adp_energy_readings r
  JOIN umut_aws_classic_stable_catalog.adp.adp_facilities f ON f.facility_id = r.facility_id
  JOIN umut_aws_classic_stable_catalog.adp.adp_emission_factors fe ON fe.grid_region = f.grid_region AND fe.fuel = 'electricity'
  JOIN umut_aws_classic_stable_catalog.adp.adp_emission_factors fg ON fg.grid_region = f.grid_region AND fg.fuel = 'gas'
  WHERE r.facility_id = facility
    AND r.reading_ts >= start_date AND r.reading_ts < end_date + INTERVAL 1 DAY
)
-- @@STATEMENT@@
CREATE OR REPLACE FUNCTION umut_aws_classic_stable_catalog.adp.target_progress(
  facility STRING COMMENT 'Facility id, e.g. FAC-001.')
RETURNS STRING
...
RETURN (
  WITH bounds AS (
    ...
    FROM umut_aws_classic_stable_catalog.adp.adp_energy_readings WHERE facility_id = facility
  )
  SELECT to_json(struct(
  ...
  JOIN umut_aws_classic_stable_catalog.adp.adp_emission_factors fe ON fe.grid_region = f.grid_region AND fe.fuel = 'electricity'
  JOIN umut_aws_classic_stable_catalog.adp.adp_emission_factors fg ON fg.grid_region = f.grid_region AND fg.fuel = 'gas'
  JOIN umut_aws_classic_stable_catalog.adp.adp_targets tg ON tg.scope = 'facility' AND tg.facility_id = r.facility_id
  CROSS JOIN bounds b
  WHERE r.facility_id = facility
